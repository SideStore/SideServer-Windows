...
const char* REGISTRY_ROOT_KEY = "SOFTWARE\\RileyTestut\\SideServer";
...
std::wstring sidestoreSourceURL = L"https://apps.sidestore.io";
std::wstring sidestoreBundleID = L"com.SideStore.SideStore";
...
{
	auto helpError = SideServerApp::instance()->helpError();
	if (helpError == NULL)
	{
		return;
	}

	std::string url("https://faq.sidestore.io/getting-started/error-codes?q=");
...
SideServerApp* SideServerApp::_instance = nullptr;

SideServerApp* SideServerApp::instance()
{
	if (_instance == 0)
	{
		_instance = new SideServerApp();
	}

	return _instance;
}

SideServerApp::SideServerApp() : _appGroupSemaphore(1)
{
	 CLSIDFromString(L"{96A5974D-D3A2-909A-B6BD-4FF84E7880F7}", &_notificationIconGUID);
}

SideServerApp::~SideServerApp()
{
}
...
std::string SideServerApp::BrowseForFolder(std::wstring title, std::string folderPath)
{
...
void SideServerApp::Start(HWND windowHandle, HINSTANCE instanceHandle)
{
	_windowHandle = windowHandle;
	_instanceHandle = instanceHandle;

#if STAGING
	win_sparkle_set_appcast_url("https://sidestore.io/sideserver/sparkle-windows-staging.xml");
#else
	win_sparkle_set_appcast_url("https://sidestore.io/sideserver/sparkle-windows.xml");
#endif
	...
	{
		// Make SideServer appear in notification area.
...
void SideServerApp::Stop()
{
	win_sparkle_cleanup();
}

void SideServerApp::CheckForUpdates()
{
	win_sparkle_check_update_with_ui();
}

pplx::task<std::shared_ptr<Application>> SideServerApp::InstallApplication(std::optional<std::string> filepath, std::shared_ptr<Device> installDevice, std::string appleID, std::string password)
{
	...
	.then([=](pplx::task<std::shared_ptr<Application>> task) -> pplx::task<std::shared_ptr<Application>> {
		...
		catch (APIError& error)
		{
			if ((APIErrorCode)error.code() == APIErrorCode::InvalidAnisetteData)
			{
				...
				// This appears to happen when iCloud is running simultaneously, and just happens to provision device at same time as SideServer.
...
pplx::task<std::shared_ptr<Application>> SideServerApp::_InstallApplication(std::optional<std::string> filepath, std::shared_ptr<Device> installDevice, std::string appleID, std::string password)
{
				...
				*certificate = *tempCertificate;
                ...
                return this->PrepareDevice(device).then([=](pplx::task<void> task) {
                    ...
                    else
                    {
                        ...
                        // Show alert before downloading SideStore.
                        this->ShowInstallationNotification("SideStore", device->name());
...
pplx::task<void> SideServerApp::PrepareDevice(std::shared_ptr<Device> device)
{
...
pplx::task<std::string> SideServerApp::FetchSideStoreDownloadURL(std::shared_ptr<Device> device)
{
	uri_builder builder(sidestoreSourceURL);
	...
	.then([=](json::value json) {
		try
		{
			...
			std::optional<json::value> sidestore = std::nullopt;
			for (auto& app : apps)
			{
				auto bundleID = app[L"bundleIdentifier"].as_string();
				if (bundleID == sidestoreBundleID)
				{
					sidestore = app;
					break;
				}
			...
			auto bundleID = StringFromWideString(sidestoreBundleID);

			if (!sidestore.has_value())
			{
			...
			if (!sidestore->has_array_field(L"versions"))
			{
			...
			auto versions = (*sidestore)[L"versions"].as_array();
			if (versions.size() == 0)
			{
			...
			{
				throw ServerError(ServerErrorCode::UnsupportediOSVersion, { {AppNameErrorKey, "SideStore"}, {OperatingSystemNameErrorKey, osName}, {OperatingSystemVersionErrorKey, minOSVersionString} });
			}
			...
			else
			{
				...
				oss << device->name() << " is running " << osName << " " << device->osVersion().stringValue() << ", but SideStore requires " << osName << " " << minOSVersion << " or later.";
				oss << "\n\n";
				oss << "Would you like to download the last version compatible with this device instead (SideStore " << StringFromWideString(latestSupportedVersionNumber) << ")?";
...
pplx::task<fs::path> SideServerApp::DownloadApp(std::shared_ptr<Device> device)
{
    ...
    auto task = fstream::open_ostream(WideStringFromString(temporaryPath.string()))
		...
		{
			*outputFile = file;
			return this->FetchSideStoreDownloadURL(device);
		})
...
pplx::task<std::pair<std::shared_ptr<Account>, std::shared_ptr<AppleAPISession>>> SideServerApp::Authenticate(std::string appleID, std::string password, std::shared_ptr<AnisetteData> anisetteData)
{
...
pplx::task<std::shared_ptr<Team>> SideServerApp::FetchTeam(std::shared_ptr<Account> account, std::shared_ptr<AppleAPISession> session)
{
...
pplx::task<std::shared_ptr<Certificate>> SideServerApp::FetchCertificate(std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
    ...
    .then([this, team, session](std::vector<std::shared_ptr<Certificate>> certificates)
          {
			...
			for (auto& certificate : certificates)
			{
				...
				std::string prefix("SideStore");

				if (certificate->machineName()->size() < prefix.size())
				{
					// Machine name doesn't begin with "SideStore", so ignore.
					continue;
				}
				else
				{
				...
				// Machine name starts with SideStore.
              ...
              {
                  std::string machineName = "SideStore";
...
pplx::task<std::map<std::string, std::shared_ptr<ProvisioningProfile>>> SideServerApp::PrepareAllProvisioningProfiles(
...
pplx::task<std::shared_ptr<ProvisioningProfile>> SideServerApp::PrepareProvisioningProfile(
...
pplx::task<std::shared_ptr<AppID>> SideServerApp::RegisterAppID(std::string appName, std::string bundleID, std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
...
pplx::task<std::shared_ptr<AppID>> SideServerApp::UpdateAppIDFeatures(std::shared_ptr<AppID> appID, std::shared_ptr<Application> app, std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
...
pplx::task<std::shared_ptr<AppID>> SideServerApp::UpdateAppIDAppGroups(std::shared_ptr<AppID> appID, std::shared_ptr<Application> app, std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
	return pplx::create_task([=]() -> pplx::task<std::shared_ptr<AppID>> {
		...
		.then([=](std::vector<std::shared_ptr<AppGroup>> fetchedGroups) {
			...
			for (auto groupIdentifier : applicationGroups)
			{
				...
				{
					std::string name = std::regex_replace("SideStore " + groupIdentifier, std::regex("\\."), " ");
...
pplx::task<std::shared_ptr<Device>> SideServerApp::RegisterDevice(std::shared_ptr<Device> device, std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
...
pplx::task<std::shared_ptr<ProvisioningProfile>> SideServerApp::FetchProvisioningProfile(std::shared_ptr<AppID> appID, std::shared_ptr<Device> device, std::shared_ptr<Team> team, std::shared_ptr<AppleAPISession> session)
{
    return AppleAPI::getInstance()->FetchProvisioningProfile(appID, device->type(), team, session);
}

pplx::task<std::shared_ptr<Application>> SideServerApp::InstallApp(std::shared_ptr<Application> app,
                            std::shared_ptr<Device> device,
                            std::shared_ptr<Team> team,
                            ...
                            std::map<std::string, std::shared_ptr<ProvisioningProfile>> profilesByBundleID)
{
    ...
    return pplx::task<std::shared_ptr<Application>>([=]() {
		...
		std::string openAppURLScheme = "sidestore-" + app->bundleIdentifier();
		...
		plist_t sidestoreURLScheme = plist_new_dict();
		plist_dict_set_item(sidestoreURLScheme, "CFBundleTypeRole", plist_new_string("Editor"));
		plist_dict_set_item(sidestoreURLScheme, "CFBundleURLName", plist_new_string(app->bundleIdentifier().c_str()));

		plist_t schemesNode = plist_new_array();
		plist_array_append_item(schemesNode, plist_new_string(openAppURLScheme.c_str()));
		plist_dict_set_item(sidestoreURLScheme, "CFBundleURLSchemes", schemesNode);

		plist_array_append_item(allURLSchemes, sidestoreURLScheme);
		...
		if (team->type() == Team::Type::Free && app->isSideStoreApp())
		{
...
pplx::task<void> SideServerApp::EnableJIT(InstalledApp app, std::shared_ptr<Device> device)
{
...
void SideServerApp::ShowNotification(std::string title, std::string message)
{
...
void SideServerApp::ShowAlert(std::string title, std::string message)
{
	MessageBoxW(NULL, WideStringFromString(message).c_str(), WideStringFromString(title).c_str(), MB_OK);
}

void SideServerApp::ShowErrorAlert(std::exception& exception, std::string localizedTitle)
{
	std::string message;
...
void SideServerApp::ShowInstallationNotification(std::string appName, std::string deviceName)
{
...
bool SideServerApp::CheckDependencies()
{
...
bool SideServerApp::CheckiCloudDependencies()
{
	wchar_t* programFilesCommonDirectory;
	SHGetKnownFolderPath(FOLDERID_ProgramFilesCommon, 0, NULL, &programFilesCommonDirectory);
...
void SideServerApp::HandleAnisetteError(AnisetteError& error)
{
...
If you already have iCloud installed, please locate the "Apple" folder that was installed with iCloud. This can normally be found at:
	...
	{
		std::wstring message = L"Please locate the 'Apple' folder installed with iTunes to continue using SideServer.\n\nThis can normally be found at:\n";
...
HWND SideServerApp::windowHandle() const
{
	return _windowHandle;
}

HINSTANCE SideServerApp::instanceHandle() const
{
	return _instanceHandle;
}

Error* SideServerApp::helpError() const
{
	return _helpError;
}

bool SideServerApp::boolValueForRegistryKey(std::string key) const
{
	auto value = GetRegistryBoolValue(key.c_str());
	return value;
}

void SideServerApp::setBoolValueForRegistryKey(bool value, std::string key)
{
	SetRegistryBoolValue(key.c_str(), value);
}

bool SideServerApp::automaticallyLaunchAtLogin() const
{
	auto value = GetRegistryBoolValue(LAUNCH_AT_STARTUP_KEY);
	return value;
}

void SideServerApp::setAutomaticallyLaunchAtLogin(bool launch)
{
	...
	if (launch)
	{
		...
		result = RegSetValueExA(hKey, "SideServer", 0, REG_SZ, (const BYTE*)executablePath, length + 1); // Must include NULL-character in size.
	}
	else
	{
		RegDeleteValueA(hKey, "SideServer");
	}

	RegCloseKey(hKey);
}

std::string SideServerApp::serverID() const
{
	auto serverID = GetRegistryStringValue(SERVER_ID_KEY);
	return serverID;
}

void SideServerApp::setServerID(std::string serverID)
{
	SetRegistryStringValue(SERVER_ID_KEY, serverID);
}

bool SideServerApp::presentedRunningNotification() const
{
...
void SideServerApp::setPresentedRunningNotification(bool presentedRunningNotification)
{
	SetRegistryBoolValue(PRESENTED_RUNNING_NOTIFICATION_KEY, presentedRunningNotification);
}

bool SideServerApp::reprovisionedDevice() const
{
	auto reprovisionedDevice = GetRegistryBoolValue(REPROVISIONED_DEVICE_KEY);
	return reprovisionedDevice;
}

void SideServerApp::setReprovisionedDevice(bool reprovisionedDevice)
{
	SetRegistryBoolValue(REPROVISIONED_DEVICE_KEY, reprovisionedDevice);
}

std::string SideServerApp::defaultAppleFolderPath() const
{
	wchar_t* programFilesCommonDirectory;
	SHGetKnownFolderPath(FOLDERID_ProgramFilesCommon, 0, NULL, &programFilesCommonDirectory);
...
std::string SideServerApp::appleFolderPath() const
{
...
void SideServerApp::setAppleFolderPath(std::string appleFolderPath)
{
	SetRegistryStringValue(APPLE_FOLDER_KEY, appleFolderPath);
}

std::string SideServerApp::internetServicesFolderPath() const
{
...
std::string SideServerApp::applicationSupportFolderPath() const
{
...
fs::path SideServerApp::appDataDirectoryPath() const
{
	wchar_t* programDataDirectory;
	SHGetKnownFolderPath(FOLDERID_ProgramData, 0, NULL, &programDataDirectory);

	fs::path sideserverDirectoryPath(programDataDirectory);
	sideserverDirectoryPath.append("SideServer");

	if (!fs::exists(sideserverDirectoryPath))
	{
		fs::create_directory(sideserverDirectoryPath);
	}

	return sideserverDirectoryPath;
}

fs::path SideServerApp::certificatesDirectoryPath() const
{
...
fs::path SideServerApp::developerDisksDirectoryPath() const
{
	auto appDataPath = this->appDataDirectoryPath();
