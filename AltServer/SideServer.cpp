...
)
{
	...
	SideServerApp::instance()->Start(hWnd, hInst);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case (WM_USER + 1):
	{
		...
		case WM_LBUTTONUP:
		{
			...
			if (SideServerApp::instance()->automaticallyLaunchAtLogin())
			{
			...
			{
				auto launchAtLogin = SideServerApp::instance()->automaticallyLaunchAtLogin();
				SideServerApp::instance()->setAutomaticallyLaunchAtLogin(!launchAtLogin);
			}
			else if (id == ID_MENU_CHECK_FOR_UPDATES)
			{
				SideServerApp::instance()->CheckForUpdates();
			}
			...
			else if (id >= FIRST_APP)
			{
				...
				auto task = SideServerApp::instance()->EnableJIT(app, device);
...
BOOL CALLBACK LoginDlgProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			auto task = SideServerApp::instance()->InstallApplication(_ipaFilepath, _selectedDevice, StringFromWideString(appleID), StringFromWideString(password));
